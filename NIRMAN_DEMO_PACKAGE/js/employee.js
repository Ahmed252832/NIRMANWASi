...
function employeeRenderDashboard() {
  ...
  var responsibilities = [
    ["AL", "Confirm booking allocations", pendingAllocations + " pending", ""],
    ["PY", "Verify client Payments", pendingPayments + " pending payment(s)", "payments.html"],
    ["CP", "Resolve client complaints", pendingComplaints + " unresolved", ""],
    ["TN", "Publish and award tenders", ownTenders.length + " published by you", "tenders.html"],
    ["PR", "Project supervision", "Check progress and deadlines", ""]
  ];
  var responsibilityHtml = "";
  for (index = 0; index < responsibilities.length; index += 1) {
    var responsibilityTitle = escapeHtml(responsibilities[index][1]);
    if (responsibilities[index][3]) {
      responsibilityTitle = '<a href="' + escapeHtml(responsibilities[index][3]) + '">' + responsibilityTitle + "</a>";
    }
    responsibilityHtml += '<li class="activity-item"><span class="activity-marker">' +
      escapeHtml(responsibilities[index][0]) + "</span><div><h3>" + responsibilityTitle +
      "</h3><p>" + escapeHtml(responsibilities[index][2]) + "</p></div></li>";
  }
...
function employeeRenderProfile() {
  ...
  if (personDetails) {
    personDetails.innerHTML = employeeDetailItem("First name", person.firstName) +
  ...
  if (employeeDetails) {
    employeeDetails.innerHTML = employeeDetailItem("Designation", employee.designation) +
  ...
  if (manager) {
    manager.innerHTML = managerId ? '<div class="activity-item"><span class="activity-marker">MG</span><div><h3>' +
      ...
      '<p class="text-muted-custom mb-0">No direct manager.</p>';
  }
  var subordinateHtml = "";
  for (index = 0; index < subordinateIds.length; index += 1) {
    ...
    subordinateHtml += '<li class="activity-item"><span class="activity-marker">SB</span><div><h3>' +
      ...
      escapeHtml(subordinate ? subordinate.designation + " - " + subordinate.deptName : "Employee") +
      "</p></div></li>";
  }
...
function employeeRenderTenders() {
  ...
  for (index = 0; index < nirmanData.tenders.length; index += 1) {
    ...
    tenderHtml += '<tr data-filter-row data-status="' + escapeHtml(tender.status) + '"><td><span class="table-primary-text">' +
      ...
      "</span></td><td>" + escapeHtml(getEmployeeName(tender.employeeId)) + "</td><td>" +
  ...
  for (index = 0; index < nirmanData.tenderBids.length; index += 1) {
    ...
    bidHtml += '<tr data-filter-row data-status="' + escapeHtml(bid.bidStatus) + '"><td><span class="table-primary-text">' +
      escapeHtml("Bid " + bid.bidId) + "</span></td><td>" +
...
function employeeOpenTenderDetail(tenderId) {
  ...
  var html = '<ul class="detail-list">' + employeeDetailItem("Tender ID", tender.tenderId) +
    employeeDetailItem("Publisher", getEmployeeName(tender.employeeId)) +
...
function employeeOpenBidDetail(compositeKey) {
  ...
  employeeSetText("tenderDetailTitle", "Bid " + bid.bidId + (tender ? " - " + tender.title : ""));
  var html = '<ul class="detail-list">' +
    employeeDetailItem("Bid", "Bid " + bid.bidId) +
    employeeDetailItem("Tender", tender ? tender.title : bid.tenderId) +
    employeeDetailItem("Representative", getRepresentativeName(bid.repId)) +
    ...
    employeeDetailItem("Resulting project", project ? project.projectName : "No resulting project") +
    "</ul>";
  var body = document.getElementById("tenderDetailBody");
  if (body) {
    body.innerHTML = html;
...
function employeeReviewBid(compositeKey, action) {
  ...
  showPageAlert("Bid marked " + nextStatus + ".", "success");
}
...
function employeeSubmitTender(event) {
  ...
  if (!window.confirm("Publish Tender " + tenderId + "?")) {
    return;
  }
  ...
  employeeRenderTenders();
  showPageAlert("Tender published successfully.", "success");
}
...
function employeeSubmitAward(event) {
  ...
  employeeRenderTenders();
  showPageAlert("Award and project created successfully.", "success");
}
...
function employeeConfirmAllocation(bookingId) {
  ...
  showPageAlert("Booking allocation confirmed successfully.", "success");
}
