  ...
  if (bidStatusList) {
    ...
    for (index = 0; index < ownBids.length; index += 1) {
      ...
      bidHtml += '<div class="activity-item"><span class="activity-marker">BD</span><div><h3>' +
        escapeHtml((bidTender ? bidTender.title : "Tender") + " - Bid " + bid.bidId) + " " + createStatusBadge(bid.bidStatus) + "</h3><p>" +
  ...
  if (projectList) {
    ...
    for (index = 0; index < relevantProjects.length; index += 1) {
      ...
      projectHtml += '<div class="activity-item"><span class="activity-marker">PR</span><div><h3>' +
        escapeHtml(project.projectName) + " " + createStatusBadge(project.status) +
        "</h3><p>Latest progress " + escapeHtml(contractorFormatProgress(latest ? latest.progressPercent : 0)) + "</p></div></div>";
    }
    projectHtml += "</div>";
