...
function employeeFetchTendersData(callback) {
  Promise.all([
    fetch("../../get_tenders_employee.php").then(function (r) { return r.json(); }),
    fetch("../../get_bids_employee.php").then(function (r) { return r.json(); }),
    fetch("../../get_areas_admin.php").then(function (r) { return r.json(); })
...
function employeeRenderDashboard() {
  fetch("../../get_employee_dashboard.php?empId=" + encodeURIComponent(currentEmployeeId))
    ...
    .then(function (data) {
      ...
      employeeSetText("dashboardTenderCount", data.ownTenders.length);
      ...
      var responsibilities = [
        ...
        ["TN", "Publish and award Tenders", data.ownTenders.length + " tender(s) published by you", "tenders.html"],
      ...
      var tenderHtml = "";
      for (var t = 0; t < data.ownTenders.length; t += 1) {
        var tender = data.ownTenders[t];
      ...
      var projectHtml = "";
      for (var p = 0; p < data.projects.length; p += 1) {
...
function employeeRenderProfile() {
  fetch("../../get_employee_profile.php?empId=" + encodeURIComponent(currentEmployeeId))
    .then(function (response) { return response.json(); })
    .then(function (data) {
      if (!data.success) {
...
function employeeBidIsEligibleForAward(bid) {
  return Boolean(bid && bid.TENDER_STATUS !== "Awarded" && bid.BID_STATUS !== "Rejected" && !bid.AWARD_ID);
}
...
function employeeReviewBid(compositeKey, action) {
  ...
  fetch("../../review_bid.php", { method: "POST", body: formData })
    .then(function (r) { return r.json(); })
...
function employeeSubmitTender(event) {
  ...
  formData.append("empId", currentEmployeeId);
  formData.append("deadline", document.getElementById("newTenderDeadline").value);
  formData.append("status", document.getElementById("newTenderStatus").value);
  ...
  fetch("../../publish_tender.php", { method: "POST", body: formData })
    .then(function (r) { return r.json(); })
    .then(function (result) {
      if (result.success) {
        form.reset();
...
function employeeShowInstallments(compositeKey) {
  ...
  employeeSetText("installmentTitle", "Installments for " + getClientName(payment.clientId) + " - Payment " + payment.paymentId);
...
function employeeVerifyPayment(compositeKey) {
  ...
  if (!window.confirm("Verify Payment " + payment.paymentId + " for " + getClientName(payment.clientId) + "?")) {
    return;
  }
...
function employeeFetchProjectsData(callback) {
  Promise.all([
    fetch("../../get_portfolio_projects.php").then(function (r) { return r.json(); }),
    fetch("../../get_portfolio_areas.php").then(function (r) { return r.json(); }),
    fetch("../../get_portfolio_updates.php").then(function (r) { return r.json(); }),
    fetch("../../get_portfolio_units.php").then(function (r) { return r.json(); }),
    fetch("../../get_awards_admin.php").then(function (r) { return r.json(); }),
    fetch("../../get_bids_admin.php").then(function (r) { return r.json(); }),
    fetch("../../get_contractors_admin.php").then(function (r) { return r.json(); }),
    fetch("../../get_supervisions_admin.php").then(function (r) { return r.json(); }),
    fetch("../../get_representatives_admin.php").then(function (r) { return r.json(); })
  ]).then(function (results) {
    employeeProjectsCache = results[0];
    employeeAreasCache2 = results[1];
    employeeUpdatesCache = results[2];
    employeeUnitsCache = results[3];
    employeeAwardsCache = results[4];
    employeeBidsForProjectsCache = results[5];
    employeeContractorsCache = results[6];
    employeeSupervisionsCache = results[7];
    employeeRepsCache = results[8];
    if (callback) {
      callback();
    }
  }).catch(function (error) {
...
function employeeRenderProjects() {
  ...
  for (index = 0; index < ownSupervisions.length; index += 1) {
    ...
    var otherSupervisors = employeeSupervisionsCache.filter(function (s) {
      return s.CONTRACTOR_ID === supervision.CONTRACTOR_ID && s.EMP_ID !== currentEmployeeId;
    }).map(function (s) { return s.FIRST_NAME + " " + s.LAST_NAME; });
  ...
  if (supervisionBody) {
    supervisionBody.innerHTML = supervisionHtml || employeeEmptyTableRow(5, "No assignments", "You do not supervise a Contractor yet.");
  }
...
function employeePopulateSupervisionChoices() {
  ...
  var ownContractorIds = employeeSupervisionsCache
    .filter(function (s) { return s.EMP_ID === currentEmployeeId; })
    .map(function (s) { return s.CONTRACTOR_ID; });

  for (index = 0; index < employeeContractorsCache.length; index += 1) {
    var contractor = employeeContractorsCache[index];
    if (ownContractorIds.indexOf(contractor.CONTRACTOR_ID) === -1) {
      eligibleCount += 1;
      html += '<option value="' + escapeHtml(contractor.CONTRACTOR_ID) + '">' +
        escapeHtml(contractor.CONTRACTOR_ID + " - " + contractor.COMPANY_NAME) + "</option>";
    }
  }
  if (eligibleCount === 0) {
...
function employeeSubmitSupervision(event) {
  ...
  if (!contractorId) {
    showPageAlert("Choose an existing Contractor not already supervised by you.", "danger");
    return;
  }
  if (!window.confirm("Assign yourself to supervise this Contractor?")) {
    return;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("../../get_current_employee.php")
    .then(function (response) { return response.json(); })
    .then(function (result) {
      if (!result.loggedIn) {
      ...
      // other pages still call the old mock-based functions for now
