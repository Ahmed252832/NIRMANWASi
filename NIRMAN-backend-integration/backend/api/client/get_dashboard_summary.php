...
$bookingStatus = client_summary_rows(
    $conn,
    "SELECT Booking_status AS Status, COUNT(*) AS Record_count
     FROM Booking WHERE Cl_id = :client_id
     GROUP BY Booking_status ORDER BY Booking_status",
    $clientId,
    'Client booking status summary failed'
);
$installmentStatus = client_summary_rows(
    $conn,
    "SELECT Status, COUNT(*) AS Record_count
     FROM Installment WHERE Cl_id = :client_id
     GROUP BY Status ORDER BY Status",
    $clientId,
    'Client installment status summary failed'
);
