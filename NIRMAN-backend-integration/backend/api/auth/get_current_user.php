...
$sql = "SELECT First_Name, Last_Name, Email, Contact_no, Profile_photo FROM Person WHERE Person_id = :p_pid";
...
if ($role === 'admin' || $role === 'employee') {
    $roleSql = "SELECT Emp_id, Designation, Dept_name FROM Employee WHERE Emp_id = :role_id AND Person_id = :person_id";
} elseif ($role === 'client') {
...
$roleRecord = oci_fetch_assoc($roleStmt);
if (!$roleRecord) {
    api_clear_identity();
    api_response(['loggedIn' => false]);
}
...
if ($role === 'admin' || $role === 'employee') {
    $response['designation'] = $roleRecord['DESIGNATION'];
    $response['deptName'] = $roleRecord['DEPT_NAME'];
    if ($role === 'employee') {
        $response['permissions'] = api_employee_access($conn, $roleId)['features'];
    }
}
