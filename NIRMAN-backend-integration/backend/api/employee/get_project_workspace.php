...
$conn = getConnection();
api_require_employee_feature($conn, 'projects');
...
$projectsSql = $scope . "
    SELECT cp.Project_id, cp.Award_id, cp.Area_id, cp.Project_Budget, cp.Project_name,
           ...
           (SELECT MAX(pu.Progress_percent) KEEP
                       (DENSE_RANK LAST ORDER BY pu.Update_date NULLS FIRST, pu.Update_id)
            FROM Project_Update pu WHERE pu.Project_id = cp.Project_id) AS Latest_progress
    FROM Construction_Project cp
