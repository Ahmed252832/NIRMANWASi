...
function api_employee_feature_map()
{
    return [
        'Operations' => ['allocations', 'tenders', 'projects'],
        'Finance' => ['payments'],
        'Engineering' => ['tenders', 'projects'],
        'Client Services' => ['complaints'],
        'Procurement' => ['tenders']
    ];
}

function api_employee_access($conn, $employeeId = null)
{
    static $cache = [];
    $employeeId = $employeeId ?: ($_SESSION['emp_id'] ?? '');
    if (isset($cache[$employeeId])) {
        return $cache[$employeeId];
    }

    $stmt = oci_parse($conn, 'SELECT Dept_name, Designation FROM Employee WHERE Emp_id = :employee_id');
    oci_bind_by_name($stmt, ':employee_id', $employeeId);
    if (!oci_execute($stmt)) {
        api_database_error($stmt, 'Employee permission lookup failed');
    }
    $row = oci_fetch_assoc($stmt);
    if (!$row) {
        api_error(403, 'Employee access could not be verified.', 'forbidden');
    }

    $map = api_employee_feature_map();
    $department = $row['DEPT_NAME'];
    $designation = $row['DESIGNATION'];
    $designationMap = [
        'Operations Manager' => ['allocations', 'tenders', 'projects'],
        'Finance Officer' => ['payments'],
        'Project Engineer' => ['tenders', 'projects'],
        'Site Engineer' => ['tenders', 'projects'],
        'Client Service Officer' => ['complaints'],
        'Procurement Officer' => ['tenders']
    ];
    $departmentFeatures = $map[$department] ?? [];
    $designationFeatures = $designationMap[$designation] ?? [];
    $cache[$employeeId] = [
        'department' => $department,
        'designation' => $designation,
        'features' => array_values(array_intersect($departmentFeatures, $designationFeatures))
    ];
    return $cache[$employeeId];
}

function api_require_employee_feature($conn, $feature)
{
    api_require_role('employee');
    $access = api_employee_access($conn);
    if (!in_array($feature, $access['features'], true)) {
        api_error(403, 'Your department and designation are not authorized for this feature.', 'feature_forbidden');
    }
    return $access;
}
