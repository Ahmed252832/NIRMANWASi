...
current frontend remains built with HTML5, CSS3, basic Bootstrap, and
beginner-friendly vanilla JavaScript. The workspace now also contains the
PHP/OCI8 backend integration under `backend/`; this document describes the
frontend source and does not change the database design.
...
Centralized `js/data.js` arrays cover the frontend fixture data:
...
- Implementation: local Bootstrap assets for grid, navigation, forms, tables,
  cards, badges, alerts, and modal behavior; a small shared custom stylesheet
  for the visual identity; simple inline SVG icons where useful.
...
- General login routes each supplied account to its matching role through the
  PHP authentication endpoints; fixture data alone does not provide security.
